...
def create_expense():
    ...
    shares = {
        str(k): int(v) for k, v in data.get("shares", {}).items()
    ...
    if group_id:
        ...
        for user_id_str in shares.keys():
            if ObjectId(user_id_str) not in group_member_ids:
                return (
                    jsonify(
                        {"message": f"User {user_id_str} is not a member of the group"}
                    ),
                    400,
                )
    ...
    if group_id:
        mongo.db.groups.update_one(
            {"_id": ObjectId(group_id)}, {"$addToSet": {"expenses": result.inserted_id}}
        )
