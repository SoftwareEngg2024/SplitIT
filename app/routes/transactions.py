...
def add_transaction():
    ...
    transaction_data = {
        ...
        "amount_usd": (
            data.get("amount_usd") if data.get("amount_usd") != None else None
        ),
    }

    # Update user's transactions
    user_transactions = mongo.db.user_transactions.find_one(
...
def set_budget():
    ...
    # Update the document
    mongo.db.user_transactions.update_one(
        {"user_id": ObjectId(user_id)}, {"$set": {"budget": budget}}, upsert=True
    )
