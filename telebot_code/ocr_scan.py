...
def run(message, bot):
    ...
    msg = bot.send_message(
        id,
        "Make sure that the letters are clearly legible. At the moment we offer the best support for Walmart receipts, however we are working on it!\nPlease provide a photo of your bill.",
    )
...
def download_file(bot, photo):
    ...
    F = requests.get(
        "https://api.telegram.org/file/bot" + str(api_token) + "/" + tempfile.file_path
    )
...
def photo_receive_handler(message, bot):
    ...
    if message.photo is None or len(message.photo) == 0:
        msg = bot.reply_to(
            message,
            "Sorry, there are no photos in the message you sent. Please try again.",
        )
        bot.register_next_step_handler(msg, photo_receive_handler, bot)
        ...
        if tempf is None:
            ...
            if exp["invalid"] == 1:
                ...
                msg = bot.reply_to(
                    message,
                    "The amounts were not correctly detected, or you might not have a compatible bill format. Do you want to enter manually?",
                    reply_markup=markup,
                )
                ...
                msg = bot.reply_to(
                    message,
                    "These are the amounts on the Scanned bill:\n"
                    + print_exp(exp)
                    + "\nAre you sure this is correct?",
                    reply_markup=markup,
                )
                bot.register_next_step_handler(
                    msg, expense_type_selection_or_retry, bot, exp
                )
...
def expense_type_selection_or_retry(message, bot, exp):
    if message.text == "Yes":
        ...
        msg = bot.reply_to(
            message,
            "Do you want this as a complete expense or a broken up expense?",
            reply_markup=markup,
        )
        ...
        markup.add("No")
        msg = bot.reply_to(
            message, "Do you want to enter manually?", reply_markup=markup
        )
        bot.register_next_step_handler(msg, post_auto_or_manual_selection, bot)
...
def post_scan_expense_type_selection(message, bot, exp):
    ...
    if message.text == helper.getScannedExpenseOptions()["complete"]:
        expenseRecord = helper.ExpenseRecord(
            title="Expense",
            date=exp["date"],
            category="Expense",
            amount=exp["total"],
        ...
        for eachexpense in exp["prices"]:
            ...
            add.add_user_expense_record(
                bot, message.from_user.id, expenseRecord.to_dict()
            )
...
def img_preprocess(img):
    if img is None:
        ...
        if receipt_contour is None:
            ...
            receipt = imutils.perspective.four_point_transform(
                img, receipt_contour.reshape(4, 2) * ratio
            )
    ...
    if img is None:
        ...
        for line in sn:
            ...
            if subtotal_ratio > 80 and total_ratio <= subtotal_ratio:
                ...
                if len(s) == 0:
                    ...
                    dictp["subtotal"] = (
                        s[0].strip().replace(" ", "").replace(",", ".").replace("$", "")
                    )
            elif tax_ratio > 70.0:
                continue
        ...
        if "total" in dictp.keys() and "subtotal" in dictp.keys():
            if isfloat(dictp["total"]) and isfloat(dictp["subtotal"]):
                dictp["tax"] = str(
                    round(float(dictp["total"]) - float(dictp["subtotal"]), 2)
                )
            else:
