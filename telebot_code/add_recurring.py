...
def post_amount_input(message, bot, selectedCurrency):
    try:
        ...
        if amount_value == 0:  # cannot be $0 spending
            ...
            message = bot.send_message(
                chat_id,
                "For how many months in the future will the expense/income be there? \n(Enter integer values only)",
            )
            bot.register_next_step_handler(
                message, post_duration_input, bot, amount_value
            )
    except Exception as e:
        logging.exception(str(e))
        bot.reply_to(message, "Oh no. " + str(e))
...
def post_duration_input(message, bot, amount_value):
    try:
        ...
        bot.send_message(
            chat_id,
            "The following expenditure has been recorded: You have spent/received ${} for {} for the next {} months. Actual currency is {} and value is {}\n".format(
                convert_value_str,
                category_str,
                duration_value,
                currency_str,
