...
def monthwise_or_daywise_record(message, bot):
    if message.text == "Daywise":
        gran = 'day'
        msg = bot.reply_to(message, "Enter the number of "+gran+"s you want to show on the graph")
...
def single_or_group_expenses(message, bot, granularity, ndays):
    ...
    if message.text == 'Mine':
        ...
        if det == None:
            ...
            for rec in exp:
                record = {'userid' : uid, 'timestamp' : rec['date'], 'expense' : rec['amount']}
                dfp.append(record)
            df = pd.DataFrame(dfp)
        ...
        markup.add("Mine")
        markup.add("Group")
        msg = bot.reply_to(message, "Only 2 options are available. Try again.\nDo you want to plot your expenses or group expenses?", reply_markup = markup)
        bot.register_next_step_handler(msg, single_or_group_expenses, bot, granularity, ndays)
