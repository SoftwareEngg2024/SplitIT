...
def update_user_transaction(telegram_user_id, update_data):
    ...
    result = collectionUserTransactions.update_one(
        {"telegram_user_id": telegram_user_id}, {"$set": update_data}
    )
    if result.matched_count > 0:
        updated_doc = collectionUserTransactions.find_one(
...
def delete_user_transaction(telegram_user_id):
    ...
    result = collectionUserTransactions.delete_one(
        {"telegram_user_id": telegram_user_id}
    )
...
def merge_user_transactions(id, telegram_user_id):
    ...
    userTransaction1.transactions["income_data"].extend(
        userTransaction2.transactions["income_data"]
    )
    userTransaction1.transactions["expense_data"].extend(
        userTransaction2.transactions["expense_data"]
    )
...
def link_user(telegram_id, email_id, link_code):
    ...
    if user == None:
        ...
        result = collectionUsers.update_one(
            {"email": email_id}, {"$set": user.to_dict()}
        )
        if result.matched_count > 0:
            ...
            res = merge_user_transactions(
                updated_user._id, updated_user.telegram_user_id
            )
...
def save_user_email(telegram_user_id, email):
    ...
    result = collectionUsers.update_one(
        {"telegram_user_id": telegram_user_id}, {"$set": {"email": email}}, upsert=True
    )
